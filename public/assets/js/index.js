...
var _libFaceTracker = require('./lib/face-tracker');

var _libFaceTracker2 = _interopRequireDefault(_libFaceTracker);

var _libFaceSubstitution = require('./lib/face-substitution');

var _libFaceSubstitution2 = _interopRequireDefault(_libFaceSubstitution);

var faceTracker = new _libFaceTracker2['default']('camera', 'overlay');
var faceSubstitution = new _libFaceSubstitution2['default'](new clm.tracker(), pModel, faceTracker, faceDeformer, Poisson);
...
},{"./lib/face-substitution":3,"./lib/face-tracker":4}],3:[function(require,module,exports){
'use strict';
...
var FaceSubstitution = (function () {
	function FaceSubstitution(clmTracker, pModel, faceTracker, faceDeformer, Poisson) {
		_classCallCheck(this, FaceSubstitution);

		this.ctrack = clmTracker;
		this.pModel = pModel;
		this.ctrack.init(this.pModel);

		this.faceTracker = faceTracker;
		this.faceDeformer = faceDeformer;
	...
	_createClass(FaceSubstitution, [{
		key: 'setMasks',
		value: function setMasks() {
			this.masks = {
		...
		value: function setImages() {
			this.images = ["average", "terminator", "walter2", "clooney2", "bieber", "kim", "rihanna", "audrey", "bill", "connery2", "cage3", "queen", "obama4", "chuck", "monalisa", "picasso1", "scream"];
		}
	}, {
		key: 'setCurrentMask',
		value: function setCurrentMask() {
			this.currentMask = 0;
		}
	}, {
		key: 'setExtendedVertices',
		value: function setExtendedVertices() {
			this.extendedVertices = [[0, 71, 72, 0], [0, 72, 1, 0], [1, 72, 73, 1], [1, 73, 2, 1], [2, 73, 74, 2], [2, 74, 3, 2], [3, 74, 75, 3], [3, 75, 4, 3], [4, 75, 76, 4], [4, 76, 5, 4], [5, 76, 77, 5], [5, 77, 6, 5], [6, 77, 78, 6], [6, 78, 7, 6], [7, 78, 79, 7], [7, 79, 8, 7], [8, 79, 80, 8], [8, 80, 9, 8], [9, 80, 81, 9], [9, 81, 10, 9], [10, 81, 82, 10], [10, 82, 11, 10], [11, 82, 83, 11], [11, 83, 12, 11], [12, 83, 84, 12], [12, 84, 13, 12], [13, 84, 85, 13], [13, 85, 14, 13], [14, 85, 86, 14], [14, 86, 15, 14], [15, 86, 87, 15], [15, 87, 16, 15], [16, 87, 88, 16], [16, 88, 17, 16], [17, 88, 89, 17], [17, 89, 18, 17], [18, 89, 90, 18], [18, 90, 22, 18], [22, 90, 21, 22], [21, 90, 91, 21], [21, 20, 91, 21], [20, 91, 92, 20], [20, 92, 19, 20], [19, 92, 93, 19], [19, 93, 71, 19], [19, 0, 71, 19], [44, 61, 56, 44], [60, 61, 56, 60], [60, 56, 57, 60], [60, 59, 57, 60], [58, 59, 57, 58], [58, 59, 50, 58]];
		}
	}, {
		key: 'setAuxCanvases',
		...
		value: function setFaceDeformers() {

			this.fd1 = new this.faceDeformer();
			this.fd2 = new this.faceDeformer();

			this.webGl1Element = document.getElementById('webgl');
			this.fd1.init(this.webGl1Element);
			this.wc1 = this.webGl1Element.getContext('webgl') || this.webGl1Element.getContext('experimental-webgl');
			this.wc1.clearColor(0, 0, 0, 0);

			this.webGl2Element = document.getElementById('webgl2');
			this.fd2.init(this.webGl2Element);
			this.wc2 = this.webGl2Element.getContext('webgl') || this.webGl2Element.getContext('experimental-webgl');
			this.wc2.clearColor(0, 0, 0, 0);
		}
	}, {
		key: 'setImageCanvases',
		value: function setImageCanvases() {
			this.imageCanvases = {};
			for (var i = 0; i < this.images.length; i++) {
				(0, _jquery2['default'])("#" + this.images[i]).load((function (obj) {
					...
					this.imageCanvases[elementId] = imageCanvas;
				}).bind(this));
			}
		}
	}, {
		...
		value: function startVideo() {
			...
			console.log('ins');
			console.log('this.videoReady: ' + this.videoReady);

			console.log('this.imagesReady: ' + this.imagesReady);
		...
		value: function switchMasks(pos) {
			...
			var newVertices = this.pModel.path.vertices.concat(this.extendedVertices);

			// deform the mask we want to use to face form
			this.fd2.load(this.imageCanvases[this.images[this.currentMask]], newMaskPos, this.pModel, newVertices);
			this.fd2.draw(newFacePos);
			// and copy onto new canvas
			this.newCanvas.getContext('2d').drawImage(this.webGl2Element, 0, 0);

			// create masking
			var tempcoords = this.positions.slice(0, 18);
			tempcoords.push(this.positions[21]);
			tempcoords.push(this.positions[20]);
			tempcoords.push(this.positions[19]);
			this.createMasking(this.maskCanvas, tempcoords);

			// do poisson blending
			Poisson.load(this.newCanvas, this.videoCanvas, this.maskCanvas, (function () {
				var result = Poisson.blend(30, 0, 0);
				// render to canvas
				this.newCanvas.getContext('2d').putImageData(result, 0, 0);
				// get mask

				var maskname = Object.keys(this.masks)[this.currentMask];
				this.fd1.load(this.newCanvas, pos, this.pModel);
				requestAnimFrame(this.drawMaskLoop);
			}).bind(this));
		}
	}, {
		key: 'drawMaskLoop',
		value: function drawMaskLoop() {
			// get position of face
    ...
    positions[i][1] += 1;
			...
			this.faceTracker.overlayCC.clearRect(0, 0, 400, 300);
			if (this.positions) {
				// draw mask on top of face
				this.fd.draw(positions);
			}
			this.animationRequest = requestAnimFrame(this.drawMaskLoop);
		}
	}, {
		key: 'createMasking',
		value: function createMasking(canvas, modelpoints) {
	...
	return FaceSubstitution;
})();

exports['default'] = FaceSubstitution;
...
var FaceTracker = (function () {
	...
	_createClass(FaceTracker, [{
		key: 'enableCameraStream',
		value: function enableCameraStream(errorCallback) {
